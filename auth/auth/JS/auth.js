    ...
    if (u) {
      area.innerHTML = `
        <div class="nav-user-pill">
          <div class="nav-user-avatar" onclick="ProfilePanel.open()" title="My Profile">
            ${u.username.charAt(0).toUpperCase()}
          </div>
          <span class="nav-username" onclick="ProfilePanel.open()">${u.username}</span>
          ${u.role === 'admin' ? `<button class="nav-login-btn" style="font-size:.6rem;padding:.2rem .5rem;border-color:var(--red);color:var(--red)" onclick="AdminPanel.open()">ADMIN</button>` : ''}
          <button class="nav-logout-btn" onclick="Auth.logout()">EXIT</button>
        </div>`;
    } else {
      area.innerHTML = `
        <button class="nav-login-btn" onclick="GatePage.show('login')">LOGIN</button>
        <button class="nav-login-btn register" onclick="GatePage.show('register')">REGISTER</button>`;
    }
  }
...
const AdminPanel = (() => {
  ...
  function renderActivityFeed(activity) {
    ...
    el.innerHTML = activity.slice(0, 30).map(a => {
      ...
      return `<div class="activity-entry">
        <span class="act-time">${a.timeLocal || a.time}</span>
        <span class="act-icon">${icon}</span>
        <span class="act-msg"><b style="color:var(--green-dim)">${user?.username || 'Unknown'}</b> — ${a.detail}</span>
      </div>`;
  ...
  function renderCountriesChart(users) {
    ...
    chart.innerHTML = sorted.map(([country, count]) => `
      <div class="admin-bar" title="${country}: ${count} user${count>1?'s':''}">
        <div class="admin-bar-fill" style="height:${Math.round((count/max)*100)}%"></div>
        <span class="admin-bar-label">${country.slice(0,3)}</span>
      </div>`).join('');
  }
